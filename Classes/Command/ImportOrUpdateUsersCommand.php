 ...
 */
class ImportOrUpdateUsersCommand extends Command
{
    /**
    ...
    public function configure()
    {
        $this
            ->setDescription(
                'Import new users or update existing ones'
            )
            ->setHelp(
                'New users will be imported from LDAP servers, existing ones will be updated with current data from LDAP servers.'
            )
            ->addArgument(
                'servers',
    ...
    public function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($ldapServers as $ldapServer) {
            if (in_array($ldapServer->getConfiguration()->getUid(), $serverUids)) {
                $io->writeln('Importing and updating from server: ' . $ldapServer->getConfiguration()->getUid());
                $runs = [];
                if ($processFe) {
